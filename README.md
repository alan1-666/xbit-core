...
curl -sS 'http://127.0.0.1:8086/v1/futures/open-orders?userAddress=0xabc'
...
The GraphQL facade currently covers the frontend's futures symbol, account, fills, open orders, Hyperliquid signing, order lifecycle, order status sync and Smart Money address-management operations. It uses seeded in-memory data and the local Hyperliquid provider locally, then switches persistence to Postgres when `POSTGRES_DSN` is set.
