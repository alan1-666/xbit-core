...
func main() {
	...
	cfg, err := config.LoadService("wallet", ":8082")
	if err != nil {
		logger.Error("load config", "error", err)
		os.Exit(1)
	}

	store := wallet.Store(wallet.NewMemoryStore())
	if cfg.DatabaseDSN != "" {
		...
		store = wallet.NewPostgresStore(postgres.Pool)
	}
	...
	wallet.NewHandler(wallet.NewService(store)).RegisterRoutes(router)
